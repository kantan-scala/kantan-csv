...
lazy val benchmark = projectMatrix
  ...
  .settings(
    TaskKey[Unit]("runBench") := Def
      .uncached((Jmh / run).toTask(" -i 10 -wi 10 -f 2 -t 1 -rf csv -rff benchmarks.csv").value),
    TaskKey[Unit]("runProfiler") := Def.uncached(
      (Jmh / run)
        .toTask(
...
lazy val jackson = projectMatrix
  ...
  .settings(
    libraryDependencies ++= Seq(
      "com.fasterxml.jackson.dataformat" % "jackson-dataformat-csv" % "2.22.3"
    )
  )
  ...
  .settings(
    libraryDependencies ++= {
      if(virtualAxes.value.toSet.contains(VirtualAxis.jvm))
        Seq("commons-io" % "commons-io" % "2.22.0" % Test)
      else
        Nil
    }
  )
...
lazy val codecsEnumeratumLaws = kantanCrossProject("codecs-enumeratum-laws", "codecs/enumeratum/laws")
  ...
  .settings(libraryDependencies += "com.beachape" %% "enumeratum-scalacheck" % "1.9.8")
  .settings(
    scalacOptions ++= {
      scalaBinaryVersion.value match {
        case "3" =>
