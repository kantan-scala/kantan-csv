...
/** Provides various instance creation and summoning methods.
  ...
  *
  *   - `decoder`: creates decoders from a function of arity `XXX` and for which you need to specify a mapping
  *     ''parameter to row index'' (such as if the order in which cells are written doesn't match that of the function's
  *     parameters).
  *   - `ordered`: create decoders from a function of arity `XXX` such that its parameters are organised in exactly the
  *     same way as CSV rows.
  *
...
object RowDecoder extends GeneratedRowDecoders with DecoderCompanion[Seq[String], DecodeError, codecs.type] {
  ...
  /** Provides a [[RowDecoder]] instance that decodes a single cell from each row.
    ...
    *   }}}
    */
  def field[A: CellDecoder](index: Int): RowDecoder[A] =
    RowDecoder.from { ss =>
